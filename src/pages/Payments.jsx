...
export default function Payments({ isBoss, session, openModal, setOpenModal }) {
  ...
  const [payType, setPayType] = useState('student') // student | general
  ...
  /** Validatsiya → yashirin parol modali */
  const requestSave = () => {
  ...
  const confirmSave = async () => {
    ...
    if (payType === 'general') {
      const paidAt = tashkentDateEndIso(form.periodTo)
      ...
      notifyTelegram('general_payment', {
        paid_amount: amountNum,
        method: form.method === 'cash' ? 'Naqd' : 'Karta',
        period: periodLabel,
  ...
  const closeModal = () => {
    ...
    setSearch('')
    setSearchResults([])
    setForm({ amount: '', method: 'cash', notes: '', periodFrom: '', periodTo: '' })
  }
  ...
  const isGeneral = (p) => !p.student_id
    ...
    <div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E5E7EB', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
        {loading ? (
          ...
          <div key={p.id} style={{ padding: '14px 18px', borderBottom: '1px solid #F9FAFB', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }} onMouseOver={e => e.currentTarget.style.background = '#FAFBFF'} onMouseOut={e => e.currentTarget.style.background = ''}>
            <div style={{
              width: 32, height: 32, borderRadius: 8,
              background: isGeneral(p) ? '#059669' : '#DC2626',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0,
            }}>
              {isGeneral(p) ? 'U' : (p.students?.full_name?.[0] || '?')}
            </div>
            <div style={{ flex: 1, minWidth: 120 }}>
              <div style={{ fontSize: 13, fontWeight: 600 }}>
                {isGeneral(p) ? "Umumiy to'lov" : (p.students?.full_name || '—')}
              </div>
              <div style={{ fontSize: 11, color: '#9CA3AF' }}>
                {isGeneral(p)
                  ? (fmtPeriod(p.period_from, p.period_to) || p.notes || '—')
                  : `${p.students?.groups?.name || '—'} · ${p.profiles?.full_name || '—'}`}
                {isGeneral(p) && p.profiles?.full_name ? ` · ${p.profiles.full_name}` : ''}
              </div>
            </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={e => e.target === e.currentTarget && closeModal()}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, boxShadow: '0 20px 60px rgba(0,0,0,.15)', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ fontFamily: 'Nunito,sans-serif', fontWeight: 800, fontSize: 18, marginBottom: 16 }}>To'lov kiritish</h2>

            <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
              {[
                { val: 'student', label: "O'quvchi to'lovi" },
                { val: 'general', label: "Umumiy to'lov" },
              ].map(t => (
                <div
                  key={t.val}
                  onClick={() => {
                  ...
                  style={{
                    flex: 1, padding: '9px', border: `1.5px solid ${payType === t.val ? '#DC2626' : '#E5E7EB'}`,
                    borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', textAlign: 'center',
                    background: payType === t.val ? '#FEF2F2' : '#fff',
                    color: payType === t.val ? '#DC2626' : '#6B7280',
                  }}
                >
                  {t.label}
                </div>
            ...
            {payType === 'student' && selectedStudent && form.amount && parseMoneyInput(form.amount) > 0 && (
            ...
            {payType === 'general' && (
              <div style={{ background: '#ECFDF5', border: '1px solid #A7F3D0', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 12, color: '#065F46' }}>
                Bu to'lov hech qaysi o'quvchiga bog'lanmaydi — faqat kassa balansiga qo'shiladi.
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={requestSave} disabled={saving} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: '#DC2626', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      {/* Parol — yashirin (type=password), prompt emas */}
      {showPassModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 110, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 400, boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
            ...
            <p style={{ fontSize: 13, color: '#6B7280', marginBottom: 20 }}>To'lovni saqlash uchun parolni kiriting.</p>
